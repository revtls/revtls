...
func CreateRootCertificate(subjectAltName string) (*x509.Certificate, *rsa.PrivateKey) {
	...
	if err := os.WriteFile("rootCACert.pem", pemCert, 0644); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote root cert.pem\n")
	...
	if err := os.WriteFile("rootCAKey.pem", pemKey, 0600); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote root CA key.pem\n")

	return &template, privateKey
}

func CreateDomainOwnerCertificate(subjectAltName string, privateKey *rsa.PrivateKey, rootTemplate x509.Certificate) (*x509.Certificate, *ecdsa.PrivateKey) {
	...
	if err := os.WriteFile("domainOwnerCert.pem", pemCert, 0644); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote domain owner cert.pem\n")
	...
	if err := os.WriteFile("domainOwnerKey.pem", pemKey, 0600); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote domain owner key.pem\n")

	return &template, domainOwnerPrivateKey
}

func CreateDomainOwnerRDC(privKeyPem []byte, domainOwnerTemplate x509.Certificate) {
	...
	privKeyDer, _ := pem.Decode([]byte(privKeyPem))
	if privKeyDer == nil {
		log.Fatal("Privkey DER decode error")
	}
	privateKey, err := x509.ParsePKCS8PrivateKey(privKeyDer.Bytes)
	if err != nil {
		log.Fatal(err)
	}
	...
	if err := os.WriteFile("domainOwnerDDC.json", jsonCert, 0644); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote domainOwnerDDC.json\n")
	...
	if err := os.WriteFile("domainOwnerDDCKey.pem", pemKey, 0600); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote domainOwnerDDCKey.pem\n")
}

func CreateDomainOwnerAndMiddleboxRDC(privKeyPem []byte, domainOwnerTemplate x509.Certificate, numOfMb int, numOfKey int) {
	...
	if err := os.WriteFile("middleboxDDC.json", jsonCert, 0644); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote middleboxDDC.json\n")
	//Temporary client DDC
	if err := os.WriteFile("clientDDC.json", jsonCert, 0644); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote clientDDC.json\n")
	...
	if err := os.WriteFile("middleboxDDCKey.pem", pemKey, 0600); err != nil {
		log.Fatal(err)
	}
	log.Print("wrote middleboxDDCKey.pem\n")
