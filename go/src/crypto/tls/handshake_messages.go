...
func (m *clientHelloMsg) marshal() []byte {
	...
	b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
		...
		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			...
			if len(m.ddcExtension) > 0 {
				b.AddUint16(extensionDDC)
				b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
					b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
						b.AddBytes([]byte(m.ddcExtension))
					})
				})
			}
			if m.ocspStapling {
...
func (m *clientHelloMsg) unmarshal(data []byte) bool {
	*m = clientHelloMsg{raw: data}
	...
	for !extensions.Empty() {
		...
		case extensionDDC:
			var ddcExtension cryptobyte.String
			if !extData.ReadUint16LengthPrefixed(&ddcExtension) {
				return false
			}
			m.ddcExtension = string(ddcExtension)
...
func (m *serverHelloMsg) marshal() []byte {
	...
	b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
		...
		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			...
			// if len(m.ddcKeyLocation) > 0 {
			// 	b.AddUint16(extensionDDC)
			// 	b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			// 		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			// 			for _, keyLocation := range m.ddcKeyLocation {
			// 				b.AddUint8LengthPrefixed(func(b *cryptobyte.Builder) {
			// 					b.AddBytes([]byte(keyLocation))
			// 				})
			// 			}
			// 		})
			// 	})
			...
			if len(m.ddcKeyLocation) > 0 {
				...
				b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
					b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
						b.AddBytes([]byte(m.ddcKeyLocation))
					})
				})
			}
...
func (m *serverHelloMsg) unmarshal(data []byte) bool {
	*m = serverHelloMsg{raw: data}
	...
	for !extensions.Empty() {
		...
		// case extensionDDC:
		// 	var ddcKeyList cryptobyte.String
		// 	if !extData.ReadUint16LengthPrefixed(&ddcKeyList) || ddcKeyList.Empty() {
		// 		return false
		// 	}
		// 	for !ddcKeyList.Empty() {
		// 		var keyLocation cryptobyte.String
		// 		if !ddcKeyList.ReadUint8LengthPrefixed(&keyLocation) || keyLocation.Empty() {
		// 			return false
		// 		}
		// 		m.ddcKeyLocation = append(m.ddcKeyLocation, string(keyLocation))
		// 	}

		case extensionDDC:
			var ddcKeyLocation cryptobyte.String
			if !extData.ReadUint16LengthPrefixed(&ddcKeyLocation) {
				return false
			}
			m.ddcKeyLocation = string(ddcKeyLocation)
...
func marshalCertificate(b *cryptobyte.Builder, certificate Certificate) {
	b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
		b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
			b.AddBytes(certificate.DDCByte)
		})
...
func unmarshalCertificate(s *cryptobyte.String, certificate *Certificate) bool {
	...
	var ddc []byte
	if !readUint24LengthPrefixed(&certList, &ddc) {
		return false
	}
	certificate.DDCByte = ddc
