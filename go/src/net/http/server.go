...
func ServeTLS(l net.Listener, handler Handler, certFile, sscFile, keyFile string, flag int) error {
	srv := &Server{Handler: handler}
	return srv.ServeTLS(l, certFile, sscFile, keyFile, flag)
}
...
func (srv *Server) ServeTLS(l net.Listener, certFile, ddcFile, keyFile string, flag int) error {
	...
	if flag == 0 {
		...
		if !configHasCert || certFile != "" || keyFile != "" {
			...
			config.Certificates[0], err = tls.LoadX509KeyPair(certFile, keyFile, flag)
			if err != nil {
				return err
			}
		}
		...
		configHasCertAndSSC := len(config.Certificates) > 0 || config.GetCertificate != nil
		if !configHasCertAndSSC || certFile != "" || ddcFile != "" || keyFile != "" {
			var err error
			config.Certificates = make([]tls.Certificate, 1)
			config.Certificates[0], err = tls.LoadX509KeyPair(certFile, keyFile, flag)
			if err != nil {
		...
		var ddc x509.DDC
		ddcJSONBlock, err := os.ReadFile(ddcFile)
		if err != nil {
			return err
		}
		json.Unmarshal(ddcJSONBlock, &ddc)
		//Add Params with considering using JSON structure
		ddc.DelegatedKeys[0].PublicKeyRaw.Curve = elliptic.P256().Params()
		config.Certificates[0].DDCByte = ddcJSONBlock
		config.DDC = &ddc
...
// of the server's certificate, any intermediates, and the CA's certificate.
func ListenAndServeTLS(addr, certFile, sscFile, keyFile string, flag int, handler Handler) error {
	server := &Server{Addr: addr, Handler: handler}
	return server.ListenAndServeTLS(certFile, sscFile, keyFile, flag)
}
...
func (srv *Server) ListenAndServeTLS(certFile, sscFile, keyFile string, flag int) error {
	if srv.shuttingDown() {
		return ErrServerClosed
	}
	addr := srv.Addr
	...
	if flag == 0 {
		return srv.ServeTLS(ln, certFile, "", keyFile, flag)
	} else if flag == 1 {
		return srv.ServeTLS(ln, certFile, sscFile, keyFile, flag)
	} else {
		return errors.New("Invalid flag")
	}
}
