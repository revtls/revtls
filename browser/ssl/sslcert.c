...
ssl_CopyServerCert(const sslServerCert *oc)
{
    ...
    if (SECITEM_CopyItem(NULL, &sc->ddc, &oc->ddc) != SECSuccess) {
        goto loser;
    }
    if (oc->ddcKeyPair) {
        sc->ddcKeyPair = ssl_GetKeyPairRef(oc->ddcKeyPair);
    }
...
ssl_FreeServerCert(sslServerCert *sc)
{
    ...
    if (sc->ddc.len) {
        SECITEM_FreeItem(&sc->ddc, PR_FALSE);
    }
    if (sc->ddcKeyPair) {
        ssl_FreeKeyPair(sc->ddcKeyPair);
    }
