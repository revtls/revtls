...
SECStatus 
tls13_ClientSendDDCXtn(const sslSocket *ss,
                       TLSExtensionData *xtnData,
                       sslBuffer *buf, PRBool *added)
{
    /* Only send the extension if support is enabled and the client can
    ...
    if (xtnData->ddcExtension){
        PORT_Free(xtnData->ddcExtension);
    }
    xtnData->ddcExtension = "DDCExtension";
    SECStatus rv = sslBuffer_AppendVariable(buf, (const PRUint8 *)xtnData->ddcExtension, strlen(xtnData->ddcExtension), 2);
    if (rv != SECSuccess) {
...
SECStatus
tls13_ClientHandleDDCKeyLocationXtn(const sslSocket *ss, TLSExtensionData *xtnData,
                                     SECItem *data)
{
    SECStatus rv;
    SECItem tmp;
    ...
    if (rv != SECSuccess) {
        printf("tls13_ClientHandleDDCKeyLocationXtn FAIL\n");
        return SECFailure;
    }

    SECITEM_FreeItem(&xtnData->ddcKeyLocation, PR_FALSE);
    rv = SECITEM_MakeItem(NULL, &xtnData->ddcKeyLocation, tmp.data, tmp.len+1);
    if (rv != SECSuccess) {
        printf("tls13_ClientHandleDDCKeyLocationXtn FAIL SECITEM_MakeItem\n");
        return SECFailure;
    }
